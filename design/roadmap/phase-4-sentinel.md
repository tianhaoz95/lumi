...
- [x] **1.3.2** Create `NotificationService` in `lib/features/sentinel/notification_service.dart`:
...
- [x] **4.1.2** Create a `BatteryMonitor` in `lib/features/sentinel/battery_monitor.dart` that logs battery level before and after each `onHeartbeat()` invocation to the `sentinel_logs` table.
- [x] **4.1.3** In the Settings screen, add a **Sentinel Health** section (dev mode only) showing:
  - Last scan timestamp.
...
- [x] **4.2.2** If scan exceeds 30 s, cancel it and log a warning. The iOS background task must complete within its allotted time to avoid being penalized by the system scheduler.
