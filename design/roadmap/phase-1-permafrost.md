...
- [ ] **2.2.1** `LumiCard` — glassmorphism card (`BackdropFilter`, `BoxDecoration` with `surface-container-lowest` at 70% opacity, `boxShadow` at 4–6% opacity).
- [ ] **2.2.2** `LumiButton` — primary CTA: full-pill shape, `gradient` from `primary` to `primary-container` at 135°, `InkWell` with scale animation (hover 1.02×, press 0.98×).
- [ ] **2.2.3** `LumiTextField` — `surface-container-high` fill, 2px ghost-border focus ring (`primary` at 40% opacity), leading icon slot.
